...
using pygmod::interop::python::IPython;
...
namespace pygmod::extensions::python
{
    ...
    shared_ptr<interop::converters::ILuaToPythonValueConverter> lua_to_python_value_converter_instance;
    shared_ptr<interop::converters::IPythonToLuaValueConverter> python_to_lua_value_converter_instance;
...
#define PY_FUNC(name) static PyObject *name(PyObject *module, PyObject *args)
    ...
    PY_FUNC(get_type)
    {
        ...
        return python_instance->py_string_from_c_string(
            lua_base_instance->GetTypeName(lua_base_instance->GetType(stack_index)));
    }
...
#undef PY_FUNC
    ...
    static PyMethodDef method_def[] = {
        {"top", top, METH_NOARGS,
         "top() -> int\n" PyDoc_STR("Returns the index of the top element in the stack."
                                    "Because indices start at 1, this result is equal to the number of elements in the "
        ...
        {"get_table", get_table, METH_VARARGS,
         PyDoc_STR("get_table(stack_index: int) -> None\n"
                   "Pushes onto the stack the value t[k], where t is the value at the given valid index and k is the "
                   "value at the top of the stack.\n\n"
                   "This function pops the key from the stack (putting the resulting value in its place). As in Lua, "
        ...
        {"set_table", set_table, METH_VARARGS,
         PyDoc_STR("set_table(stack_index: int) -> None\n"
                   "Does the equivalent to t[k] = v, where t is the value at the given valid index, v is the value at "
                   "the top of the stack, and k is the value just below the top.\n\n"
                   "This function pops both the key and the value from the stack. As in Lua, this function may trigger "
                   "a metamethod for the \"newindex\" event.")},
        {"set_field", set_field, METH_VARARGS,
         PyDoc_STR("set_field(stack_index: int, name: str) -> None\n"
                   "Does the equivalent to t[k] = v, where t is the value at the given valid index and v is the value "
                   "at the top of the stack.\n\n"
                   "This function pops the value from the stack. As in Lua, this function may trigger a metamethod for "
                   "the \"newindex\" event.")},
        ...
        {"call", call, METH_VARARGS,
         PyDoc_STR(
             "call(args: int, results: int) -> None\n"
             "Calls a function.\n\n"
             "To call a function you must use the following protocol: first, the function to be called is pushed onto "
             "the stack; "
             "then, the arguments to the function are pushed in direct order; that is, the first argument is pushed "
             "first. "
             "Finally you call '_luastack.call'; 'args' is the number of arguments that you pushed onto the stack.\n\n"
             "All arguments and the function value are popped from the stack when the function is called. "
             "The function results are pushed onto the stack when the function returns. The number of results is "
             "adjusted to 'results', unless 'results' is -1. "
             "In this case, all results from the function are pushed. The function results are pushed onto the stack "
             "in direct order "
    ...
    static PyModuleDef module_def = {
        PyModuleDef_HEAD_INIT, "_luastack",
        PyDoc_STR(
            ...
            "We say that an index is valid if it lies between 1 and the stack top (that is, if 1 <= abs(index) <= "
            "top)."),
        0, method_def};
    ...
    void init()
    {
        ...
        {
            throw std::logic_error("lua_to_python_value_converter_instance was not set by calling "
                                   "set_lua_to_python_value_converter_instance");
        }

        if (!python_to_lua_value_converter_instance)
        {
            throw std::logic_error("python_to_lua_value_converter_instance was not set by calling "
                                   "set_python_to_lua_value_converter_instance");
        }
