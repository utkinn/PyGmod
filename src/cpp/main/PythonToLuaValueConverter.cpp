...
namespace pygmod::converters
{
	void PythonToLuaValueConverter::convert(PyObject* obj)
	{
		...
		else
		{
			...
			Py_INCREF(obj);
			const auto py_object_userdata_container = static_cast<UserData*>(lua->NewUserdata(sizeof(UserData)));
			py_object_userdata_container->data = reinterpret_cast<void*>(obj);
			if (PyCallable_Check(obj)) {
				py_object_userdata_container->type = LUA_TYPE_PYCALLABLE;
				lua->CreateMetaTableType("PyCallable", LUA_TYPE_PYCALLABLE);
			}
			else {
				py_object_userdata_container->type = LUA_TYPE_PYOBJECT;
